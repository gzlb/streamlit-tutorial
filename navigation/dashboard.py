...
def pageII():
    ...
    variance = round(coin_df['Close'].var(), 3)

    # Candle and volume chart
    fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.1, row_heights=[100, 30])
    fig.add_trace(
        go.Candlestick(x=coin_df.index, open=coin_df['Open'], high=coin_df['High'],
                       low=coin_df['Low'], close=coin_df['Close'], name='Candlestick'),
        row=1, col=1
    )
...
def get_historical_dask(coin: str, start_date, end_date, interval='1d'):
    ...
    for chunk_start in pd.date_range(start=start_date, end=end_date, freq='1M'):
        ...
        historical_dask = dd.concat([historical_dask, dd.from_pandas(chunk_data, npartitions=1)], axis=0)
